...
function cleanora_register_theme_js_scripts() {
	...
	wp_localize_script('cleanora-theme-script', 'cmsmasters_theme_script', $cmsmasters_localize_array);
}

add_action('wp_enqueue_scripts', 'cleanora_register_theme_js_scripts');



/* Scripts for Admin */
function cleanora_theme_admin_scripts() {
	$cmsmasters_option = cleanora_get_global_options();
	...
	wp_localize_script('cmsmasters-theme-settings-toggle', 'cmsmasters_theme_settings', array( 
		'shortname' => 	'cleanora', 
		'header_style' => $cmsmasters_option['cleanora' . '_header_styles'] 
	));
}

add_action('admin_enqueue_scripts', 'cleanora_theme_admin_scripts');



/* Register Theme Navigations */
register_nav_menus(array( 
    'primary' => 	esc_html__('Primary Navigation', 'cleanora'), 
...
function cleanora_fallback_menu($args) {
	$cmsmasters_option = cleanora_get_global_options();
	...
	echo '<div class="navigation_wrap">' . "\n" . 
		'<ul id="navigation" class="' . 
			'navigation ' . 
			($cmsmasters_option['cleanora' . '_header_styles'] == 'default' ? 'mid_nav' : 'bot_nav') . 
		'">' . "\n";
...
function cleanora_pagination_prev_text() {
	return esc_html__('Previous', 'cleanora');
}

add_filter('cleanora_pagination_prev_text_filter', 'cleanora_pagination_prev_text');


/* Pagination Next Text */
function cleanora_pagination_next_text() {
	return esc_html__('Next', 'cleanora');
}

add_filter('cleanora_pagination_next_text_filter', 'cleanora_pagination_next_text');



/* Single Project and Profile Layout */
function cleanora_single_project_layout($cmsmasters_layout) {
	if (
		is_singular('project') || 
		is_singular('profile') 
	) {
...
add_filter('cmsmasters_theme_page_layout_filter', 'cleanora_single_project_layout');



/* Search Form */
function cleanora_search_form($form) {
	$form = "<div class=\"search_bar_wrap\">
		<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<p class=\"search_field\">
				<input name=\"s\" placeholder=\"" . esc_attr__('enter keywords', 'cleanora') . "\" value=\"\" type=\"search\" />
			</p>
...
add_filter('get_search_form', 'cleanora_search_form');
