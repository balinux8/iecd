...
function cleanora_slider_post_heading($cmsmasters_id, $type = 'post', $tag = 'h1', $link_redirect = false, $link_url = false, $show = true, $link_target = false) { 
	if ($type == 'post') {
...
function cleanora_slider_post_exc_cont($type = 'post', $show = true) {
	if ($type == 'post') {
...
function cleanora_slider_post_check_exc_cont($type = 'post') {
	$exc = cleanora_slider_post_exc_cont($type, false);
...
function cleanora_get_slider_post_date($type = 'post', $show = true) {
	if ($type == 'post') {
...
function cleanora_get_slider_post_author($type = 'post', $show = true) {
	if ($type == 'post') {
		$out = '<span class="cmsmasters_slider_post_author">' . 
			esc_html__('by', 'cleanora') . ' ' .
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'cleanora') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($type == 'project') {
		$out = '<span class="cmsmasters_slider_project_author">' . 
			esc_html__('by', 'cleanora') . ' ' .
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Services by', 'cleanora') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	}
...
function cleanora_get_slider_post_category($cmsmasters_id, $taxonomy, $type = 'post', $show = true) {
	$out = '';
...
function cleanora_slider_post_like($type = 'post', $show = true) {
	$out = '';
...
function cleanora_get_slider_post_comments($type = 'post', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($type == 'post') {
			$out = cleanora_get_comments('cmsmasters_slider_post_comments');
		} elseif ($type == 'project') {
			$out = cleanora_get_comments('cmsmasters_slider_project_comments');
		}
	}
...
function cleanora_slider_post_more($cmsmasters_id, $type = 'post', $show = true) {
	if ($type == 'post') {
		$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
		
		
		if ($cmsmasters_post_read_more == '') {
			$cmsmasters_post_read_more = esc_attr__('Read more', 'cleanora');
		}
		...
		if ($cmsmasters_project_read_more == '') {
			$cmsmasters_project_read_more = esc_attr__('Read more', 'cleanora');
		}
		
		
		$out = '<a class="cmsmasters_slider_project_read_more" href="' . esc_url(get_permalink($cmsmasters_id)) . '">' . esc_html($cmsmasters_project_read_more) . '</a>';
	}
