...
if (!function_exists('cleanora_get_custom_all_meta_fields')) {
function cleanora_get_custom_all_meta_fields() {
	$custom_meta_fields = cleanora_get_custom_general_meta_fields();
	...
	if ( 
		...
		$custom_page_meta_fields = cleanora_get_custom_page_meta_fields();
		...
		$custom_project_meta_fields = cleanora_get_custom_project_meta_fields();
		...
		$custom_profile_meta_fields = cleanora_get_custom_profile_meta_fields();
		...
		$custom_post_meta_fields = cleanora_get_custom_post_meta_fields();
		
		$custom_all_meta_fields = array_merge($custom_post_meta_fields, $custom_meta_fields);
	} else {
		$custom_other_meta_fields = cleanora_get_custom_other_meta_fields();
...
function cleanora_admin_enqueue_scripts($hook) {
	if ( 
		($hook == 'post.php') || 
		($hook == 'post-new.php') 
	) {
		...
		wp_enqueue_style('cleanora-options-css', get_template_directory_uri() . '/framework/admin/options/css/cmsmasters-theme-options.css', array(), '1.0.0', 'screen');
		
		
		if (is_rtl()) {
			wp_enqueue_style('cleanora-options-css-rtl', get_template_directory_uri() . '/framework/admin/options/css/cmsmasters-theme-options-rtl.css', array(), '1.0.0', 'screen');
		}
		
		
		wp_enqueue_script('cleanora-options-js', get_template_directory_uri() . '/framework/admin/options/js/cmsmasters-theme-options.js', array('jquery'), '1.0.0', true);
		
		wp_localize_script('cleanora-options-js', 'cmsmasters_options', array( 
			'palettes' => 			implode(',', cmsmasters_color_picker_palettes()), 
			'create_gallery' => 	esc_attr__('Create Gallery', 'cleanora'), 
			'select_format' => 		esc_attr__('Please select the format.', 'cleanora'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'cleanora'), 
			'want_remove' => 		esc_html__('Do you really want to remove this item?', 'cleanora'), 
		...
		wp_enqueue_script('cleanora-options-js-toggle', get_template_directory_uri() . '/framework/admin/options/js/cmsmasters-theme-options-toggle.js', array('jquery'), '1.0.0', true);
	}
}

add_action('admin_enqueue_scripts', 'cleanora_admin_enqueue_scripts');


function cleanora_show_meta_box() {
	global $post;
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'cleanora') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'cleanora') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'cleanora') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'cleanora') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'cleanora') . '</option>';
		...
		case 'image':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_select_parent">' . 
				'<input type="button" id="cmsmasters_upload_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . esc_attr__('Choose Image', 'cleanora') . '" data-title="' . esc_attr__('Choose Image', 'cleanora') . '" data-button="' . esc_attr__('Insert Image', 'cleanora') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-select-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] === 'post') ? ' data-state="insert"' : '') . ' data-multiple="' . $field['multiple'] . '" />' . 
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'cleanora') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'cleanora') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'contact':
			...
			'<div class="contact_info_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Add info', 'cleanora') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon cmsmasters_remove_icon_contact_info admin-icon-remove" title="' . esc_attr__('Cancel changes', 'cleanora') . '"></a>' . 
				'</p>' . 
				...
				'</span>' . 
				'<span class="cl"></span>' . 
				'<input class="button button-primary" type="button" id="add_contact_info" value="' . esc_attr__('Add Contact Info', 'cleanora') . '" />' . 
				'<input class="button button-primary" type="button" id="edit_contact_info" value="' . esc_attr__('Save Contact Info', 'cleanora') . '" />' . 
				'<ul>';
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'cleanora') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'cleanora') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . esc_attr__('Blank Field', 'cleanora') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'cleanora') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . esc_attr__('Posts', 'cleanora') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Pages', 'cleanora') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Projects', 'cleanora') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'cleanora') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'cleanora') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'cleanora') . ' &nbsp;</option>';
		...
		case 'images_list':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_gallery_parent">' . 
				'<input type="button" id="cmsmasters_gallery_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . (($meta != '') ? esc_attr__('Edit Gallery', 'cleanora') : esc_attr__('Create Gallery', 'cleanora')) . '" data-title="' . esc_attr__('Create/Edit Gallery', 'cleanora') . '" data-button="' . esc_attr__('Insert Gallery', 'cleanora') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-gallery-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] == 'post') ? ' data-state="' . (($meta != '') ? 'gallery-edit' : 'gallery-library') . '"' : '') . ' data-multiple="' . $field['multiple'] . '"' . (($meta != '') ? ' data-editing="true"' : '') . ' />' . 
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsmasters_gallery_item">' . 
							...
							'<a href="#" class="cmsmasters_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'cleanora') . '"></a>' . 
						'</li>';
					}
				}
			}
		...
		case 'funds':
			...
			echo '<div class="cmsmasters_funds_stats">' . 
				'<div class="cmsmasters_funds_stat_wrap">' . 
					...
					'</div>' . 
				'</div>' . 
				'<span class="fl">' . esc_html__('count', 'cleanora') . ' - <strong>' . $funds_number . '</strong></span>' . 
				'<span class="fr">' . cmsmasters_donations_currency(cmsmasters_number_format($target - $funds)) . ' ' . esc_html__('to go', 'cleanora') . '</span>';
			'</div>' . 
			(($field['desc'] != '') ? '<br />' . '<span class="description">' . $field['desc'] . '</span>': '');
