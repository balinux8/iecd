...
add_action('admin_menu', 'cleanora_add_menu');
add_action('admin_init', 'cleanora_register_settings');
...
function cleanora_get_settings() {
	$output = array();
	
	
	$page = cleanora_get_admin_page();
	...
	case 'cmsmasters-settings':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE;
		$cmsmasters_settings_page_title = esc_html__('Theme General Settings', 'cleanora');
		$cmsmasters_page_sections = cleanora_options_general_sections();
		$cmsmasters_page_fields = cleanora_options_general_fields();
		$cmsmasters_page_tabs = cleanora_options_general_tabs();
	...
	case 'cmsmasters-settings' . '-font':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_font';
		$cmsmasters_settings_page_title = esc_html__('Theme Fonts Settings', 'cleanora');
		$cmsmasters_page_sections = cleanora_options_font_sections();
		$cmsmasters_page_fields = cleanora_options_font_fields();
		$cmsmasters_page_tabs = cleanora_options_font_tabs();
	...
	case 'cmsmasters-settings' . '-color':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_color';
		$cmsmasters_settings_page_title = esc_html__('Theme Color Schemes', 'cleanora');
		$cmsmasters_page_sections = cleanora_options_color_sections();
		$cmsmasters_page_fields = cleanora_options_color_fields();
		$cmsmasters_page_tabs = cleanora_options_color_tabs();
	...
	case 'cmsmasters-settings' . '-element':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_element';
		$cmsmasters_settings_page_title = esc_html__('Theme Elements Settings', 'cleanora');
		$cmsmasters_page_sections = cleanora_options_element_sections();
		$cmsmasters_page_fields = cleanora_options_element_fields();
		$cmsmasters_page_tabs = cleanora_options_element_tabs();
	...
	case 'cmsmasters-settings' . '-single':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_single';
		$cmsmasters_settings_page_title = esc_html__('Theme Single Posts Settings', 'cleanora');
		$cmsmasters_page_sections = cleanora_options_single_sections();
		$cmsmasters_page_fields = cleanora_options_single_fields();
		$cmsmasters_page_tabs = cleanora_options_single_tabs();
	...
	case 'cmsmasters-settings' . '-demo':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_demo';
		$cmsmasters_settings_page_title = esc_html__('Theme Settings Import/Export', 'cleanora');
		$cmsmasters_page_sections = cleanora_options_demo_sections();
		$cmsmasters_page_fields = cleanora_options_demo_fields();
		$cmsmasters_page_tabs = cleanora_options_demo_tabs();
...
function cleanora_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		esc_html__('Default Field', 'cleanora'), 
		'desc' => 		esc_html__('This is a default description.', 'cleanora'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function cleanora_register_settings() {
	$settings_output = cleanora_get_settings();
	...
	register_setting($cmsmasters_option_name, $cmsmasters_option_name, 'cleanora_validate_options');
	
	
	if (!empty($settings_output['cmsmasters_page_sections'])) {
		foreach ($settings_output['cmsmasters_page_sections'] as $id => $title) {
			add_settings_section($id, $title, 'cleanora_section_fn', __FILE__);
		}
	}
	...
	if (!empty($settings_output['cmsmasters_page_fields'])) {
		foreach ($settings_output['cmsmasters_page_fields'] as $option) {
			cleanora_create_settings_field($option);
		}
	}
...
function cleanora_settings_scripts() {
	wp_enqueue_style('wp-jquery-ui-dialog');
	
	
	wp_enqueue_style('cleanora-settings-css', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('cleanora-settings-css-rtl', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings-rtl.css', array(), '1.0.0', 'screen');
	}
	...
	wp_enqueue_script('cleanora-settings-js', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings.js', array('jquery', 'farbtastic'), '1.0.0', true);
	
	
	$theme_style_name = '';
	
	if (CMSMASTERS_THEME_STYLE_COMPATIBILITY) {
		foreach (cleanora_all_theme_styles() as $theme_style) {
	...
	wp_localize_script('cleanora-settings-js', 'cmsmasters_setting', array( 
		'palettes' => 						implode(',', cmsmasters_color_picker_palettes()), 
		'remove' => 						esc_attr__('Remove', 'cleanora'), 
		'remove_sidebar' => 				esc_html__('Do you really want to remove this sidebar?', 'cleanora'), 
		'find' => 							esc_attr__('Find icons', 'cleanora'), 
		'remove_icon' => 					esc_html__('Do you really want to remove this social icon?', 'cleanora'), 
		'theme_uri' => 						get_template_directory_uri(), 
		'shortname' => 						'cleanora', 
		'theme_style_name' => 				$theme_style_name, 
		'done' => 							esc_html__('All theme settings were imported successfully.', 'cleanora'), 
		'fail' => 							esc_html__("Error on page!!!\nPlease reload page and try again.", 'cleanora'), 
		'done_reset_settings' => 			esc_html__('Theme style settings were reset to default.', 'cleanora'), 
	...
	wp_enqueue_script('cleanora-settings-js-toggle', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('cleanora-settings-js-toggle', 'cmsmasters_settings', array( 
		'shortname' => 	'cleanora' 
	));
...
function cleanora_add_menu() {
	...
	$add_menu_p( 
		esc_attr__('Theme Settings', 'cleanora'), 
		esc_attr__('Theme Settings', 'cleanora'), 
		'manage_options', 
		'cmsmasters-settings', 
		'cleanora_settings_page_fn', 
		'' 
	);
	...
	$cmsmasters_settings_general = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme General Settings', 'cleanora'), 
		esc_attr__('General', 'cleanora'), 
		'manage_options', 
		'cmsmasters-settings', 
		'cleanora_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_general, 'cleanora_settings_scripts');
	
	
	$cmsmasters_settings_font = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Fonts Settings', 'cleanora'), 
		esc_attr__('Fonts', 'cleanora'), 
		'manage_options', 
		'cmsmasters-settings' . '-font', 
		'cleanora_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_font, 'cleanora_settings_scripts');
	
	
	$cmsmasters_settings_color = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Color Schemes', 'cleanora'), 
		esc_attr__('Colors', 'cleanora'), 
		'manage_options', 
		'cmsmasters-settings' . '-color', 
		'cleanora_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_color, 'cleanora_settings_scripts');
	
	
	$cmsmasters_settings_element = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Elements Settings', 'cleanora'), 
		esc_attr__('Elements', 'cleanora'), 
		'manage_options', 
		'cmsmasters-settings' . '-element', 
		'cleanora_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_element, 'cleanora_settings_scripts');
	
	
	$cmsmasters_settings_single = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Single Posts Settings', 'cleanora'), 
		esc_attr__('Single Posts', 'cleanora'), 
		'manage_options', 
		'cmsmasters-settings' . '-single', 
		'cleanora_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_single, 'cleanora_settings_scripts');
	
	
	$cmsmasters_settings_demo = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Settings Import/Export', 'cleanora'), 
		esc_attr__('Import/Export', 'cleanora'), 
		'manage_options', 
		'cmsmasters-settings' . '-demo', 
		'cleanora_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_demo, 'cleanora_settings_scripts');
}


function cleanora_section_fn($desc) {
	$tab = cleanora_get_the_tab();
...
function cleanora_settings_page_fn() {
	$settings_output = cleanora_get_settings();
	$current_tab = cleanora_get_the_tab();
	
	echo '<div class="wrap">';
	
	cleanora_settings_page_header();
	...
	if ($current_tab != 'wpml') {
		echo '<form action="options.php" method="post" class="cmsmasters_admin_page"' . (($current_tab == 'recaptcha') ? ' style="background-color:#fdffc6; padding:35px 0 15px 25px; margin:0;"' : '') . '>' . 
			'<p class="submit-top">' . 
				(($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'cleanora') . '" />' : '') . 
			'</p>';
			...
			echo '<p class="submit">';
				...
				if ($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') {
					echo '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'cleanora') . '" />';
				} elseif ($current_tab == 'import') {
					echo '<input name="import" type="button" class="button button-primary button-large cmsmasters-demo-import" value="' . esc_attr__('Import Settings', 'cleanora') . '" />';
				} elseif ($current_tab == 'theme_style') {
					echo '<input name="apply-theme-style" type="submit" class="button button-primary button-large cmsmasters-apply-theme-style" value="' . esc_attr__('Apply This Style', 'cleanora') . '" />';
					echo '<input name="reset-settings" type="button" class="button button-large cmsmasters-reset-settings" value="' . esc_attr__('Reset Theme Style Settings to Default', 'cleanora') . '" />';
				}
...
function cleanora_form_field_fn($args = array()) {
	extract($args);
	...
	case 'select_theme_style':
		$theme_style_option = get_option('cmsmasters_cleanora_theme_style');
		
		echo '<select id="' . $id . '" class="select' . $field_class . '" name="cmsmasters_cleanora_theme_style">';
	...
	case 'upload':
		...
		echo '<div class="cmsmasters_upload_parent cmsmasters_select_parent">' . 
			'<input type="button" id="cmsmasters_upload_' . $id . '_button" class="cmsmasters_upload_button button button-large" value="' . esc_attr__('Choose Image', 'cleanora') . '" data-title="' . esc_attr__('Choose Image', 'cleanora') . '" data-button="' . esc_attr__('Insert Image', 'cleanora') . '" data-id="cmsmasters-media-select-frame-' . $id . '" data-classes="media-frame cmsmasters-media-select-frame' . ((!isset($description)) ? ' cmsmasters-frame-no-description' : '') . ((!isset($caption)) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($align)) ? ' cmsmasters-frame-no-align' : '') . ((!isset($link)) ? ' cmsmasters-frame-no-link' : '') . ((!isset($size)) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $frame . '"' . (($frame == 'post') ? ' data-state="insert"' : '') . ' data-multiple="' . $multiple . '" />' . 
			'<div class="cmsmasters_upload"' . (($image != '') ? ' style="display:block;"' : '') . '>' . 
				'<img src="' . (($image != '') ? $image : '') . '" class="cmsmasters_preview_image" />' . 
				'<a href="#" class="cmsmasters_upload_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'cleanora') . '"></a>' . 
			'</div>' . 
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (cleanora_system_fonts_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_system_font'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'cleanora') . '</label>' . 
			'</div>';
		}
		...
		if ($google_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'cleanora') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'cleanora') . ' <em>(' . esc_html__('in pixels', 'cleanora') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'cleanora') . ' <em>(' . esc_html__('in pixels', 'cleanora') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'cleanora') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'cleanora') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'cleanora') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (cleanora_text_decoration_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_text_decoration'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'cleanora') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'cleanora') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'cleanora') . '" />' . 
				'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'cleanora') . '"></a>' . 
			'</p>' . 
			'<span class="cl"><br /></span>' . 
			'<span class="icon_upload_link" style="display:none;">' . 
				'<label for="new_icon_color">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Color', 'cleanora') . '</span>' . 
					'<div>' . 
				...
				'<label for="new_icon_hover">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Hover Color', 'cleanora') . '<br /><em>' . esc_html__('if not selected - Icon Color will be used', 'cleanora') . '</em></span>' . 
					'<div>' . 
				...
				'<label for="new_icon_link">' . 
					...
					esc_html__('Icon Link', 'cleanora') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					esc_html__('Icon Title', 'cleanora') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					esc_html__('Open link in a new tab/window?', 'cleanora') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'cleanora') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'cleanora') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'cleanora') . '"></a> ' . 
...
function cleanora_validate_options($input) {
	$valid_input = array();
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Expecting a Numeric value!', 'cleanora');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						'cleanora' . '_txt_numeric_error',
						esc_html__('Expecting a Numeric value! Please fix.', 'cleanora'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : esc_html__('Expecting comma separated numeric values', 'cleanora');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						$option['id'],
						'cleanora' . '_txt_multinumeric_error',
						esc_html__('Expecting comma separated numeric values! Please fix.','cleanora'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : esc_html__('Invalid email! Please re-enter!', 'cleanora');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = esc_html__('This setting field cannot be empty! Please enter a valid email address.', 'cleanora');
				}
				
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						$option['id'],
						'cleanora' . '_txt_email_error',
						esc_html__('Please enter a valid email address.', 'cleanora'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Number!', 'cleanora');
...
function cleanora_show_msg($message, $msgclass = 'info') {
	echo '<div id="message" class="' . $msgclass . '">' . $message . '</div>';
}


function cleanora_admin_msgs() {
	global $pagenow;
	...
	if ($pagenow == 'admin.php' && current_user_can('manage_options') && $cmsmasters_settings_pg !== false && !empty($set_errors)) {
		if ($set_errors[0]['code'] == 'settings_updated' && isset($_GET['settings-updated'])) {
			cleanora_show_msg('<p><strong>' . $set_errors[0]['message'] . '</strong></p>', 'updated');
		} else {
			foreach ($set_errors as $set_error) {
				cleanora_show_msg('<p class="setting-error-message" title="' . esc_attr($set_error['setting']) . '">' . $set_error['message'] . '</p>', 'error');
			}
		}
	}
}

add_action('admin_notices', 'cleanora_admin_msgs');


function cleanora_add_global_options($reset = false) {
	$cmsmasters_option_names = array( 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_general', 
			cleanora_options_general_fields('general') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_bg', 
			cleanora_options_general_fields('bg') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_header', 
			cleanora_options_general_fields('header') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_content', 
			cleanora_options_general_fields('content') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_footer', 
			cleanora_options_general_fields('footer') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_font_content', 
			cleanora_options_font_fields('content') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_font_link', 
			cleanora_options_font_fields('link') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_font_nav', 
			cleanora_options_font_fields('nav') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_font_heading', 
			cleanora_options_font_fields('heading') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_font_other', 
			cleanora_options_font_fields('other') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_element_sidebar', 
			cleanora_options_element_fields('sidebar') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_element_icon', 
			cleanora_options_element_fields('icon') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_element_lightbox', 
			cleanora_options_element_fields('lightbox') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_element_sitemap', 
			cleanora_options_element_fields('sitemap') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_element_error', 
			cleanora_options_element_fields('error') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_element_code', 
			cleanora_options_element_fields('code') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_element_recaptcha', 
			'' 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_single_post', 
			cleanora_options_single_fields('post') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_single_project', 
			cleanora_options_single_fields('project') 
		), 
		array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_single_profile', 
			cleanora_options_single_fields('profile') 
		) 
	);
	...
	foreach (cleanora_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, array( 
			'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_color_' . $key, 
			cleanora_options_color_fields($key) 
		));
	}
		...
		if ($cmsmasters_option_name[1] !== '') {
			foreach ($cmsmasters_option_name[1] as $selected_option) {
				if ( 
					...
					$selected_option['id'] !== 'cleanora' . '_social_icons' 
				) {
...
function cleanora_get_global_options() {
	...
	foreach (cleanora_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, 'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_color_' . $key);
	}
