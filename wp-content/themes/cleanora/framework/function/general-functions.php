...
/* Register CSS Styles */
function cleanora_register_css_styles() {
	if (!is_admin()) {
		global $post, 
			$wp_styles;
		...
		wp_enqueue_style('cleanora-theme-style', get_template_directory_uri() . '/style.css', array(), '1.0.0', 'screen, print');
		
		wp_enqueue_style('cleanora-style', get_template_directory_uri() . '/theme-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/style.css', array(), '1.0.0', 'screen, print');
		
		wp_enqueue_style('cleanora-adaptive', get_template_directory_uri() . '/theme-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/adaptive.css', array(), '1.0.0', 'screen, print');
		
		wp_enqueue_style('cleanora-retina', get_template_directory_uri() . '/theme-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/retina.css', array(), '1.0.0', 'screen');
		
		
		if (is_rtl()) {
			wp_enqueue_style('cleanora-rtl', get_template_directory_uri() . '/theme-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/rtl.css', array(), '1.0.0', 'screen');
		}
		...
		// iLightBox skins
		$ilightbox_skin = $cmsmasters_option['cleanora' . '_ilightbox_skin'];
		...
		if (is_dir($style_dir) && get_option('cmsmasters_style_exists_' . 'cleanora')) {
			$cmsmasters_styles_dir = $upload_dir['baseurl'] . '/cmsmasters_styles/';
		}
		
		
		wp_enqueue_style('cleanora-fonts-schemes', $cmsmasters_styles_dir . 'cleanora' . '.css', array(), '1.0.0', 'screen');
	}
}

add_action('wp_enqueue_scripts', 'cleanora_register_css_styles');



/* Register JS Scripts */
function cleanora_register_js_scripts() {
	global $post;
	...
	$cmsmasters_localize_array = array( 
		...
		'project_puzzle_proportion' => 			cleanora_project_puzzle_proportion(), 
		'gmap_api_key' => 						$cmsmasters_option['cleanora' . '_gmap_api_key'], 
		'gmap_api_key_notice' => 				esc_html__('Please add your Google Maps API key', 'cleanora'), 
		'gmap_api_key_notice_link' => 			esc_html__('read more how', 'cleanora'), 
		'primary_color' => 						$cmsmasters_option['cleanora' . '_default_link'], 
		'ilightbox_skin' => 					$cmsmasters_option['cleanora' . '_ilightbox_skin'], 
		'ilightbox_path' => 					$cmsmasters_option['cleanora' . '_ilightbox_path'], 
		'ilightbox_infinite' => 				$cmsmasters_option['cleanora' . '_ilightbox_infinite'], 
		'ilightbox_aspect_ratio' => 			$cmsmasters_option['cleanora' . '_ilightbox_aspect_ratio'], 
		'ilightbox_mobile_optimizer' => 		$cmsmasters_option['cleanora' . '_ilightbox_mobile_optimizer'], 
		'ilightbox_max_scale' => 				$cmsmasters_option['cleanora' . '_ilightbox_max_scale'], 
		'ilightbox_min_scale' => 				$cmsmasters_option['cleanora' . '_ilightbox_min_scale'], 
		'ilightbox_inner_toolbar' => 			$cmsmasters_option['cleanora' . '_ilightbox_inner_toolbar'], 
		'ilightbox_smart_recognition' => 		$cmsmasters_option['cleanora' . '_ilightbox_smart_recognition'], 
		'ilightbox_fullscreen_one_slide' => 	$cmsmasters_option['cleanora' . '_ilightbox_fullscreen_one_slide'], 
		'ilightbox_fullscreen_viewport' => 		$cmsmasters_option['cleanora' . '_ilightbox_fullscreen_viewport'], 
		'ilightbox_controls_toolbar' => 		$cmsmasters_option['cleanora' . '_ilightbox_controls_toolbar'], 
		'ilightbox_controls_arrows' => 			$cmsmasters_option['cleanora' . '_ilightbox_controls_arrows'], 
		'ilightbox_controls_fullscreen' => 		$cmsmasters_option['cleanora' . '_ilightbox_controls_fullscreen'], 
		'ilightbox_controls_thumbnail' => 		$cmsmasters_option['cleanora' . '_ilightbox_controls_thumbnail'], 
		'ilightbox_controls_keyboard' => 		$cmsmasters_option['cleanora' . '_ilightbox_controls_keyboard'], 
		'ilightbox_controls_mousewheel' => 		$cmsmasters_option['cleanora' . '_ilightbox_controls_mousewheel'], 
		'ilightbox_controls_swipe' => 			$cmsmasters_option['cleanora' . '_ilightbox_controls_swipe'], 
		'ilightbox_controls_slideshow' => 		$cmsmasters_option['cleanora' . '_ilightbox_controls_slideshow'], 
		'ilightbox_close_text' => 				esc_html__('Close', 'cleanora'), 
		'ilightbox_enter_fullscreen_text' => 	esc_html__('Enter Fullscreen (Shift+Enter)', 'cleanora'), 
		'ilightbox_exit_fullscreen_text' => 	esc_html__('Exit Fullscreen (Shift+Enter)', 'cleanora'), 
		'ilightbox_slideshow_text' => 			esc_html__('Slideshow', 'cleanora'), 
		'ilightbox_next_text' => 				esc_html__('Next', 'cleanora'), 
		'ilightbox_previous_text' => 			esc_html__('Previous', 'cleanora'), 
		'ilightbox_load_image_error' => 		esc_html__('An error occurred when trying to load photo.', 'cleanora'), 
		'ilightbox_load_contents_error' => 		esc_html__('An error occurred when trying to load contents.', 'cleanora'), 
		'ilightbox_missing_plugin_error' => 	esc_html__("The content your are attempting to view requires the <a href='{pluginspage}' target='_blank'>{type} plugin<\/a>.", 'cleanora') 
	);
...
if (!function_exists('cleanora_theme_google_fonts_generate')) {

function cleanora_theme_google_fonts_generate() {
	$cmsmasters_option = cleanora_get_global_options();
	...
	foreach (cmsmasters_google_fonts_list() as $key => $value) {
		if ( 
			(isset($cmsmasters_option['cleanora' . '_content_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_content_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_link_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_link_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_nav_title_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_nav_title_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_nav_dropdown_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_nav_dropdown_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_h1_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_h1_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_h2_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_h2_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_h3_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_h3_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_h4_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_h4_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_h5_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_h5_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_h6_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_h6_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_button_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_button_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_small_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_small_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_input_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_input_font_google_font'] && $key != '') || 
			(isset($cmsmasters_option['cleanora' . '_quote_font_google_font']) && $key == $cmsmasters_option['cleanora' . '_quote_font_google_font'] && $key != '') 
		) {
			$keys[] = $key;
		}
	}
...
add_action('wp_enqueue_scripts', 'cleanora_theme_google_fonts_generate');
...
function cleanora_the_widgets_init() {
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_default', 
            'description' => esc_html__('Widgets in this area will be shown in all left and right sidebars till you don\'t use custom sidebar.', 'cleanora'), 
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_bottom', 
            'description' => esc_html__('Widgets in this area will be shown at the bottom of middle block below the content and middle sidebar, but above footer.', 'cleanora'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_archive', 
            'description' => esc_html__('Widgets in this area will be shown in all left and right sidebars on archives pages.', 'cleanora'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_search', 
            'description' => esc_html__('Widgets in this area will be shown in left or right sidebar on search page.', 'cleanora'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
...
function cleanora_create_file($file, $content = '', $verifycontent = true) {
	global $wp_filesystem;
...
function cmsmasters_get_tweets($username, $count) {
	...
	$numTweets = $count;
	$cacheTime = 1;
	$backupName = 'cmsmasters_' . 'cleanora' . '_tweets_list_backup';
	
	
	$connection = new TwitterOAuth( 
		(($cmsmasters_option['cleanora' . '_api_key'] != '') ? $cmsmasters_option['cleanora' . '_api_key'] : ''), 
		(($cmsmasters_option['cleanora' . '_api_secret'] != '') ? $cmsmasters_option['cleanora' . '_api_secret'] : ''), 
		(($cmsmasters_option['cleanora' . '_access_token'] != '') ? $cmsmasters_option['cleanora' . '_access_token'] : ''), 
		(($cmsmasters_option['cleanora' . '_access_token_secret'] != '') ? $cmsmasters_option['cleanora' . '_access_token_secret'] : '')
	);
...
function cleanora_logo() {
	...
	if ($cmsmasters_option['cleanora' . '_logo_type'] == 'text') {
		if ($cmsmasters_option['cleanora' . '_logo_title'] != '') {
			$blog_title = $cmsmasters_option['cleanora' . '_logo_title'];
		} else {
		...
		if ($cmsmasters_option['cleanora' . '_logo_subtitle'] != '') {
			$blog_descr = $cmsmasters_option['cleanora' . '_logo_subtitle'];
		} else {
		...
		echo '<a href="' . esc_url(home_url('/')) . '" title="' . esc_attr($blog_title) . '" class="logo">' . "\n\t" . 
			'<span class="logo_text_wrap">' . 
				...
				($cmsmasters_option['cleanora' . '_logo_subtitle'] ? '<span class="title_text">' . esc_html($blog_descr) . '</span>' : '') . 
			'</span>' . 
		'</a>';
	} else {
...
function cleanora_footer_logo($cmsmasters_option) {
	...
	if ($cmsmasters_option['cleanora' . '_footer_logo_url'] == '') {
		...
		$footer_logo_img = explode('|', $cmsmasters_option['cleanora' . '_footer_logo_url']);
		...
		if ($cmsmasters_option['cleanora' . '_footer_logo_url_retina'] != '') {
			$footer_logo_img_retina = explode('|', $cmsmasters_option['cleanora' . '_footer_logo_url_retina']);
...
function cleanora_get_comments($class = false, $show = false, $add_html = false) { 
	...
	if (comments_open()) {
		$out .= '<span class="cmsmasters_comments' . ($class ? ' ' . $class : '') . '">' . 
			'<a class="cmsmasters_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'cleanora') . ' ' . esc_attr(get_the_title()) . '">' . 
...
function cleanora_theme_page_layout_scheme() {
	$cmsmasters_option = cleanora_get_global_options();
	...
	} elseif (CMSMASTERS_TRIBE_EVENTS && tribe_is_event_query() && isset($cmsmasters_option['cleanora' . '_events_layout'])) {
		$cmsmasters_layout = $cmsmasters_option['cleanora' . '_events_layout'];
	} elseif (is_archive() || is_home()) {
		$cmsmasters_layout = $cmsmasters_option['cleanora' . '_archives_layout'];
	} elseif (is_search()) {
		$cmsmasters_layout = $cmsmasters_option['cleanora' . '_search_layout'];
	} else {
		$cmsmasters_layout = $cmsmasters_option['cleanora' . '_other_layout'];
	}
	
	
	if ($cmsmasters_layout == '') {
		$cmsmasters_layout = $cmsmasters_option['cleanora' . '_layout'];
	}
...
function cleanora_global_shortcodes_styles_move($content) {
	return preg_replace("/(?<!['\"])<style.*?>([^`]*?)<\/style>/", '', $content);
}


function cleanora_row_checker($data) {
	$out = cleanora_rowcheck($data);
	
	return $out;
}

add_filter('the_content', 'cleanora_row_checker', 10, 2);
	...
	if( wp_verify_nonce($nonce, 'cmsmasters_ajax_like-nonce') ){
		...
		if ($post_ID != '') {
			...
			if (isset($_COOKIE['like-' . $post_ID]) || count($ipCheck) != 0) {
				...
				if ($add_html == 'true') {
					$text = ($plusLike == '1') ? esc_html__('like', 'cleanora') : esc_html__('likes', 'cleanora');
				} else {
					$text = '';
				}
...
function cmsmasters_ajax_apply_theme_style_callback() {
	...
	if (wp_verify_nonce($nonce, 'cmsmasters_ajax_apply_theme_style-nonce')) {
		if (isset($_POST['theme_style'])) {
			update_option('cmsmasters_cleanora_theme_style', $_POST['theme_style']);
		}
...
function cleanora_theme_style_change() {
	if (get_option('cmsmasters_options_cleanora' . CMSMASTERS_THEME_STYLE . '_theme_style')) {
		return;
	} else {
		cleanora_add_global_options();
		
		cleanora_add_global_icons();
		
		cleanora_regenerate_styles();
	}
}

add_action('init', 'cleanora_theme_style_change');
...
function cmsmasters_ajax_reset_settings_callback() {
	...
	if (wp_verify_nonce($nonce, 'cmsmasters_ajax_reset_settings-nonce')) {
		cleanora_add_global_options(true);
...
function cmsmasters_ajax_export_settings_callback() {
	...
	if (wp_verify_nonce($nonce, 'cmsmasters_ajax_export_settings-nonce')) {
		...
		foreach (cleanora_all_color_schemes_list() as $key => $value) {
			$cmsmasters_global_colors[] = 'cmsmasters_options_' . 'cleanora' . CMSMASTERS_THEME_STYLE . '_color_' . $key;
		}
