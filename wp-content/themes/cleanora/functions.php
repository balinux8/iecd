...
if (!function_exists('cleanora_system_fonts_list')) {
	function cleanora_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('cleanora_get_google_fonts_list')) {
	function cleanora_get_google_fonts_list() {
		$fonts = array( 
			'' => esc_html__('None', 'cleanora'), 
		...
		return apply_filters('cleanora_google_fonts_list_filter', $fonts);
	}
}
...
if (!function_exists('cleanora_text_decoration_list')) {
	function cleanora_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'cleanora'), 
			'underline' => esc_html__('underline', 'cleanora'), 
			'overline' => esc_html__('overline', 'cleanora'), 
			'line-through' => esc_html__('line-through', 'cleanora'), 
		);
...
if (!function_exists('cleanora_custom_color_schemes_list')) {
	function cleanora_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'cleanora'), 
			'second' => esc_html__('Custom 2', 'cleanora'), 
			'third' => esc_html__('Custom 3', 'cleanora') 
		);
		
		
		return apply_filters('cleanora_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('cleanora_locate_template')) {
	function cleanora_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_cleanora_theme_style') ? get_option('cmsmasters_cleanora_theme_style') : ''));
...
if (!function_exists('cleanora_all_theme_styles')) {
	function cleanora_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('cleanora_all_color_schemes_list')) {
	function cleanora_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'cleanora'), 
			'header' => 		esc_html__('Header', 'cleanora'), 
			'navigation' => 	esc_html__('Navigation', 'cleanora'), 
			'header_top' => 	esc_html__('Header Top', 'cleanora'), 
			'footer' => 		esc_html__('Footer', 'cleanora') 
		);
...
if (!function_exists('cleanora_load_theme_textdomain')) {
	function cleanora_load_theme_textdomain() {
		load_theme_textdomain('cleanora', get_template_directory() . '/theme-vars/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'cleanora_load_theme_textdomain')) {
	add_action('after_setup_theme', 'cleanora_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('cleanora_theme_activation')) {
	function cleanora_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'cleanora') {
			add_option('cmsmasters_active_theme', 'cleanora', '', 'yes');
...
add_action('after_switch_theme', 'cleanora_theme_activation');



// Framework Deactivation
if (!function_exists('cleanora_theme_deactivation')) {
	function cleanora_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'cleanora_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('cleanora_plugin_activation')) {
	function cleanora_plugin_activation() {
		update_option('cmsmasters_plugin_activation', 'true');
	}
}

add_action('activated_plugin', 'cleanora_plugin_activation');


if (!function_exists('cleanora_plugin_activation_regenerate')) {
	function cleanora_plugin_activation_regenerate() {
...
add_action('init', 'cleanora_plugin_activation_regenerate');
